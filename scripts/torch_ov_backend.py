...
import scripts.ov_model_state as ovm

model_state = ovm.model_state
...
def openvino_fx(subgraph, example_inputs):
    try:
        ...
        core = Core()
        if os.getenv("OPENVINO_TORCH_MODEL_CACHING") != "0" or os.getenv("OPENVINO_TORCH_MODEL_CACHING") is not None:
